...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  ...
  if (i === -1 || !process.argv[i + 1]) throw new Error('Usage: node scripts/check-skill-context.mjs --base <PR-base-commit>');
  const report = measureContext(process.cwd(), process.argv[i + 1]);
