 ...
 *
 * Usage:
 *   tsx dev/scripts/caveman-stats.ts            # ascending: compressed first
 *   tsx dev/scripts/caveman-stats.ts --desc     # descending: uncompressed first
 *   tsx dev/scripts/caveman-stats.ts --by-gain  # by absolute article count
...
if (byGain) {
  ...
  console.log(`\ntsx dev/scripts/caveman.ts ${top.join(" ")}`);
} else if (desc) {
  ...
  console.log(`\ntsx dev/scripts/caveman.ts ${suspect.map((r) => r.path).join(" ")}`);
}
