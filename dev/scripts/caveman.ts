 ...
 *
 * Usage:
 *   dev/node_modules/.bin/tsx dev/scripts/caveman.ts <file> [<file> ...]   # from the repo root
 *   dev/node_modules/.bin/tsx dev/scripts/caveman.ts skills/**\/SKILL.md     # zsh expands the glob
 */
...
// Load `.env` / `.env.local` from dev/, regardless of the cwd the
// script is invoked from.
const DEV_ROOT = resolve(import.meta.dirname, "..");
loadEnv({
  path: [resolve(DEV_ROOT, ".env.local"), resolve(DEV_ROOT, ".env")],
});
...
function main(): void {
  ...
  if (args.length === 0) {
    console.error("Usage: tsx dev/scripts/caveman.ts <file> [<file> ...]");
    process.exit(1);
  }
