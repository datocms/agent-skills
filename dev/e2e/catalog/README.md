...
Install `dev/` dependencies and the pinned fixture dependency sets needed for the selected case. Chrome must be installed; `E2E_BROWSER_CHANNEL` can select another Playwright browser channel.

```sh
npm ci --ignore-scripts --prefix dev
npm ci --ignore-scripts --prefix dev/e2e/catalog/plugin
npm ci --ignore-scripts --prefix dev/e2e/catalog/web
node dev/e2e/catalog/plugin.mjs --output local/catalog-plugin/run-01
node dev/e2e/catalog/plugin.mjs --variant sidebar-modal --output local/catalog-plugin/modal-01
```
...
For visual editing, select `--framework nextjs`, `nuxt`, `sveltekit`, or `astro` (default `nextjs`). Install the matching `dev/e2e/catalog/web-FRAMEWORK` dependency set with `npm ci`; Next.js uses `dev/e2e/catalog/web`. All frameworks verify the same user outcomes. React/Vue/Svelte must update in place; Astro may use its documented QueryListener page reload. Rechecks must specify the original framework and can rebind environment/model IDs in framework source files without changing application logic.
...
```sh
dev/node_modules/.bin/tsx dev/e2e/catalog/hosted-editor.mjs \
  --site PROJECT_ID --organization ORGANIZATION_ID \
...
```sh
node dev/e2e/catalog/plugin.mjs \
