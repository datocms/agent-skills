...
`session.json` records `oracleAccess`, the commands and edited paths that name the checkout, the evidence directory, files beside the workspace or the workspace's parent; runners that fail on credential output fail on it too. It is a post-run heuristic (the tool-chosen working directory is not recorded), not a sandbox.
