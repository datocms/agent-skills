...
The agent receives the confirmed environment and CLI authentication context. Credentials are supplied only through environment variables. The runner installs this checkout's skills in an isolated workspace, disables host configuration/plugins/memory, uses temporary authentication links, and removes its workspace and authentication directory afterward. Unless a caller passes its own `HOME`, the actor gets a fresh one (so host CLI logins stay out of reach) whose login shells resolve a stub `open`, so no actor can open the operator's browser. Shell environment snapshots are disabled so credentials are not copied into runtime snapshot files. Transcripts are redacted; observed credential output fails the run.
...
```bash
npm --prefix dev run e2e:frontend -- --repetitions 3 --output local/frontend/run-01
npm --prefix dev run e2e:code -- --repetitions 3 --output local/code/run-01
npm --prefix dev run e2e:workflows -- --repetitions 3 --output local/workflows/run-01
```
...
```bash
node dev/evals/coexistence/run.mjs \
...
Live runs default to `local/e2e/<timestamp>/<case>/`; set a fresh `E2E_RUN_ID` or `E2E_OUTPUT` to organize runs. Other suites require fresh output directories. Existing transcripts are never overwritten. Evidence includes prompts, skill hashes, revision, exact model/effort, runtime version, command events, errors, usage, independent assertions, and final answers. `skillReads` lists the skill files the actor's commands named; runners flag an empty list as "no skill consulted", since that failure says nothing about skill content. A source hash identifies uncommitted candidates more precisely than HEAD alone. Raw artifacts are ignored by Git; commit a sanitized report and coverage map instead. Superseded reports are deleted from `reports/`; git history keeps them.

`session.json` records `oracleAccess`, the commands and edited paths that name the checkout, the evidence directory, files beside the workspace or the workspace's parent. Every runner fails a session that has any, except the deployed actor, which reports it in `implementation-result.json` for the operator. It is a post-run heuristic (the tool-chosen working directory is not recorded), not a sandbox.
...
```bash
npm --prefix dev run test:e2e:harness
npm --prefix dev run typecheck
npm --prefix dev run test:coexistence:fixtures
node --test dev/tests/cma-content-safety.test.mjs dev/tests/redirect-validation.test.mjs dev/tests/optional-mcp.test.mjs dev/tests/hosted-mcp.test.mjs
```
