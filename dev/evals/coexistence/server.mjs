...
function schemaText(state) {
  let text = `Project: ${TARGET.site_id}; environment: ${TARGET.environment}. Model article, API key article, generated type Schema.Article, draft_mode_active: true (updates create unpublished revisions without changing the published version). Fields: title string (nonlocalized), untouched string, body localized structured_text (en and it). Blocks: image-block with caption and nullable image. \n${role === "datocms" ? mcpDeclarations : declarations}`;
