...
Current validation is recorded in [Terra medium validation](TERRA-REVIEW.md): 48/48 required controlled checks, 47/48 strict script-quality checks, and 6/6 retained real CLI checks. The strict gate is still false. The results below preserve earlier revisions and models; their pass counts are historical, not a current production sign-off.
...
An initial harness assertion incorrectly treated real version IDs as numeric counters. It was corrected by listing the same record's versions; the write was not rerun. Final reads also verified that none of the six records changed after its test. Cleanup deleted the temporary fork, including both added models and all six records, and verified that only the original primary environment remained at that time. Its locale list was preserved. The primary model count changed from four to one during the run; on 2026-09-14 the project owner confirmed that other tests were running in the same project. This resolves the observation as concurrent test activity. Neither test model appeared in primary, and no attempt was made to restore or otherwise alter the other tests' data. Subsequent cleanup checks track only the environment and objects created by this evaluator. The original live E2E suite and hosted-client checks remain separate.

The synthetic runs exercise tool selection, generated scripts, state preservation, and transcript/context accounting. They do not establish client installation behavior, OAuth handshakes, production server execution, or universal model reliability. The original hosted-client and E2E attempts stopped at their missing prerequisites. The additional CLI tests above used a separately authorized throwaway-project token; they do not resolve the hosted connection failure. After testing, the staged files and local outputs passed an exact-token scan, and the temporary credential file and six scratch workspaces were removed. The project token itself was not revoked.
