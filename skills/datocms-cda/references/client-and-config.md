...
| - | - | - | - |
| `token` | `string` | — | **Required.** DatoCMS API token. The library sends it as `Authorization: Bearer <token>`. Must have at least "Access the Content Delivery API" permission enabled. Additionally enable "Access the Content Delivery API in Preview Mode" only if using `includeDrafts: true`. Custom tokens scoped to specific models hide all other models from the GraphQL schema (including introspection). Legacy projects (created before 2024-01-08, "Improved GraphQL Security" not activated): inaccessible fields stay in schema, return `null`. |
