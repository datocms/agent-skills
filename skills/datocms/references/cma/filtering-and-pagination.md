...
`client.auditLogEvents` uses cursor pagination (not offset/limit) and has no `listPagedIterator`. See `resource-gotchas.md` § Audit log events for the `rawQuery` + `meta.next_token` loop.
