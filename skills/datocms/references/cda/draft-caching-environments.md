...
**Note:** The `cda_cache_tags` / `invalidate` event type does not support webhook filters — it always fires for all tag changes. To programmatically create this webhook via the CMA, see [resource gotchas](../cma/resource-gotchas.md) and [the cache-tags recipe](../../recipes/frontend-foundation/cache-tags/recipe.md).
...
For the full implementation pattern (replacement `executeQuery`, DB abstraction, webhook route handler), see `../frontend/nextjs.md` → "Cache Tags (Optional)".
