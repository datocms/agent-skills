...
`item_item_type` (link), `items_item_type` (links), `structured_text_links` carry **cascade strategies** — see `../cma/schema.md` § Reference-cascade strategies. Set allowlist deliberately; default to `"fail"` for editorial content.
...
**Set `false` when** the field's value is consumed verbatim — keys, codes, class names, slugs-in-text, IDs, `<input>` values, or anything piped to external systems — where invisible stega chars break exact-match/validation. Opts the field out at the source instead of wrapping every downstream read in `stripStega()` (see frontend-integrations `../frontend/content-link-concepts.md`).
