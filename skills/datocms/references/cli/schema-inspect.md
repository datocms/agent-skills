...
- Follow-up task is **code that consumes the schema** (typed CMA client work, record mutations, structured-text block traversal) → switch to [CMA guide](../cma.md).
- Follow-up task is **generating TypeScript types** from the schema → `schema:generate` (see `schema-generate.md`).
- Follow-up task is **changing the schema** → see the Step 2.5 decision tree in `../cli.md` (migration via [CLI guide](../cli.md) vs direct mutation via `cma:call` / `cma:script`).
