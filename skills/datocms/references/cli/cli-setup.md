...
- interactive project work uses OAuth (`login` + `link`); unattended runtime/CI uses a scoped environment token
- which profile ids
...
After the user has selected the target project, link its ID:

```bash
npx datocms projects:list blog --json
npx datocms link --site-id=<CONFIRMED_SITE_ID>
```

A single or high-confidence search result does not select the user's target. Show candidate name, ID, and workspace when selection is unresolved; wait for that choice before linking. Reuse a target already explicitly selected in the conversation.
