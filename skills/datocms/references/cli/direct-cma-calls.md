...
- **Checked-in `buildClient()` script** — unattended runtime (CI/app server/webhook/automation) needs env token. See the [CMA guide](../cma.md).
...
### Schema changes need an approach decision

`create`/`update`/`destroy` on `itemTypes`, `fields`, fieldsets, or block models changes schema. Use the canonical [schema-change workflow](schema-changes.md) to preserve the user's migration/direct choice and resolve target/authorization before execution.
...
Default page size varies by resource. Iterating all pages? Switch to [CMA guide](../cma.md) (JS client provides `listPagedIterator`).
...
- **Code runs unattended** (CI/app server/webhook/automation) needs env CMA token → checked-in `buildClient()` script via [CMA guide](../cma.md).
- **Need tests/custom error handling/retries/progress reporting** → repo script, [CMA guide](../cma.md).
