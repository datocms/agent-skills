...
> Schema mutations through `cma:script` do not create a migration history. Use the canonical [schema-change workflow](schema-changes.md) to select the approach and resolve target/authorization before execution. Preserve choices already made by the user.
...
Use a file for longer one-offs, local helper imports, or rerunning by filename. The CLI validates source structure before execution, then loads the file through `tsx` from its original location. It does not spawn the managed workspace, run `tsc --noEmit`, or inject ambient globals. Use the project's editor/typecheck for compiler feedback against its actual imports and types.
...
Both modes validate source before execution: explicit `any`/`unknown`, casts to `never`, and `@ts-ignore`/`@ts-expect-error`/`@ts-nocheck` directives are rejected. File mode requires a default async function; stdin requires top-level code. Stdin additionally runs `tsc --noEmit` inside the CLI workspace. Use precise types such as `Schema.*` for record operations:
...
- `--skip-validation` disables source validation in both modes and the stdin TypeScript check. Use only to diagnose a confirmed validation false positive, not to hide a type error.
- `--rebuild-workspace` wipes and rebuilds the internal workspace, then exits without running a script. Use after a CLI upgrade if stdin scripts fail with module-resolution errors, then invoke the script again.

File mode's source validation is not a TypeScript compiler check. Its compiler feedback comes from the project's editor or an explicit `tsc --noEmit`; missing fields/imports can otherwise fail at runtime. File-mode imports are unrestricted by the package allowlist and resolve against the script's own `node_modules`.
...
file-mode: no managed deps, no injected globals — explicit `import`s + own `package.json`. Unresolved import → `tsxRequire` error at runtime. Need a package excluded from stdin-mode globals + don't want to install? Switch to repo script (see [CMA guide](../cma.md)).
...
| Migration ([CLI guide](../cli.md)) | Code that must be **committed, versioned, and replayed** across environments. Use `migrations:new` to scaffold — file-mode script can be promoted with `mv` since imports and signature already match. |
| Checked-in `buildClient()` script ([CMA guide](../cma.md)) | **Unattended runtime** code: CI, app server, webhook, long-lived automation. Needs CMA token in environment. |
