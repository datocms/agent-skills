...
| `platform` | `cli-bootstrap`, `cma-types`, `webhooks`, `build-triggers` | project linking, schema types, webhook sync, build trigger management |
...
- **`cma-types`** — Queue alongside `cli-bootstrap` for complete first-time DatoCMS TypeScript setup. Preserve the existing type-generation convention during narrower tasks; do not expand an unrelated change into this baseline.
...
## Related topics without recipes

Some intents have no setup recipe. Use the relevant internal topic guide without inventing additional setup work.

| Intent | Sibling skill |
| - | - |
| Schema design, model vs block decisions, field shapes, taxonomy, content reuse, page-shaped schema refactor | [Modeling guide](../modeling.md) |

Greenfield gate "create new project" → queue [Modeling guide](../modeling.md) after creation, before any frontend recipe. In-flight request to add/modify models → pause recipe, route to [Modeling guide](../modeling.md), resume after schema decisions land.

## Prerequisite Rules

- Queue `draft-mode` before `web-previews`, `content-link`, or `realtime` when that foundation is missing.
- For `visual-editing`, verify `draft-mode` and `content-link` and apply only missing prerequisites; include `web-previews` unless website-only click-to-edit was chosen. Add `realtime` only on explicit or confirmed live-update intent.
...
- Keep follow-up suggestions inside [Setup guide](../setup.md) and refer to recipe ids only.
...
- Reuse the packaged domain references directly rather than copying their content into recipes.
