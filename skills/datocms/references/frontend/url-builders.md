...
Pattern for "given a record, what is its URL" when project uses `gql.tada` (see `../cda/fragment-patterns.md` for masking discipline). Apply when project already has `lib/datocms/gqlUrlBuilder/` (or similar) — don't impose otherwise.
...
Composing requires every nested fragment in the second arg (composition array) — see `../cda/fragment-patterns.md`.
