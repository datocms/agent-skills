_Internal recipe for [Setup guide](../../../references/setup.md). Use this file only after the parent skill selects the `cli-bootstrap` recipe and queues any prerequisites from `../../../references/setup/recipe-manifest.json`._
...
Follow shared repo inspection conventions in `../../../references/setup/repo-conventions.md`, then inspect recipe-specific signals below.
...
Follow zero-question default and question-format rules in `../../../references/setup/mandatory-rules.md`.

Use the project-selection policy in `../../../references/cli/cli-setup.md`. Ask when the user has not already selected the target, even if search returns one candidate. Do not treat a fuzzy match or single result as authorization to link.
...
Reuse an explicit target choice already made in the conversation; do not ask again for the same choice.

## Step 3: Load References

Read only these references:

- `../../../references/cli/cli-setup.md`
...
1. **Install `datocms`** if missing. Use project package manager (see `../../../references/setup/mandatory-rules.md`). Install as `devDependency` — CLI is development-time tool.
...
4. **Pick the right project.**
   - One result → show the candidate and resolve target selection if not already explicit.
   - Multiple results → use the user's explicit selection or ask (see Step 2).
...
- Never silently auto-pick a target from search results. Preserve the user's explicit project choice.
...
4. **TypeScript baseline** — queue `cma-types` when this bootstrap is part of complete first-time DatoCMS TypeScript setup. For a narrow linking task, preserve existing type-generation conventions and leave unrelated setup optional.
...
Follow shared final handoff rules in `../../../references/setup/output-status.md`, including explicit `Unresolved placeholders` section.
