_Internal recipe for [Setup guide](../../../references/setup.md). Use only after parent skill selects `web-previews` recipe and queues prerequisites from `../../../references/setup/recipe-manifest.json`._
...
See `../../../references/setup/output-status.md` for output status definitions.
...
1. **Framework and file layout** — use `../../../references/setup/repo-conventions.md` for framework detection, `src/` usage, standard draft-mode or preview route locations.
...
- `../../../references/frontend/web-previews-concepts.md`
...
4. Install body: two CMA calls — `client.plugins.create({ package_name: 'datocms-plugin-web-previews' })` then `client.plugins.update(plugin.id, { parameters: { frontends, startOpen: true } })`. See `../../../references/frontend/web-previews-concepts.md` § Programmatic install.
