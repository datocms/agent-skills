_Internal recipe for [Setup guide](../../../references/setup.md). Use this file only after the parent skill selects the `visual-editing` recipe and queues any prerequisites from `../../../references/setup/recipe-manifest.json`._
...
- `../../../references/frontend/realtime-concepts.md` only when the bundle decision includes real-time updates
...
1. **Verify `draft-mode` and apply it only if missing**
2. **Verify `content-link` and apply it only if missing**, unless the conflict-resolution decision preserved an existing Vercel-only website overlay flow
