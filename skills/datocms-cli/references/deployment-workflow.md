...
- **Token:** appends `--api-token` from the variable the CLI would read — the profile's `apiTokenEnvName` in `datocms.config.json` (or `DATOCMS_CONFIG_FILE`), else `DATOCMS_API_TOKEN` (default profile) or `DATOCMS_<PROFILE_ID>_PROFILE_API_TOKEN` — when set, since linked profiles never read env vars; unset → local OAuth session. `DATOCMS_PROFILE`, `DATOCMS_CONFIG_FILE` and tokens read from the process environment only, not `.env.local`/`.env` (the CLI reads both): `DATOCMS_PROFILE` in an env file → pass `--profile=<id>` explicitly, else the default profile's token rides every command and `--api-token` wins over the profile's project.
