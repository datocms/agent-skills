...
1. **Install the plugin if missing**: `npx datocms plugins:install @datocms/cli-plugin-contentful` — not `npm install`; a project dependency never registers `contentful:import`. (The `datocms` npm package is installed by `cli-bootstrap`.)
...
1. `npx datocms plugins --json` lists `@datocms/cli-plugin-contentful`
