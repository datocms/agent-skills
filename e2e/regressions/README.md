...
Install the fixture dependencies first: `npm ci --ignore-scripts` at the root and with `--prefix` for `e2e/catalog/plugin`, `web`, `web-astro` and `web-sveltekit`. Chrome is required for browser checks; the CDA case installs its pinned fixture under ignored `local/` on first use, and the importer case needs npm registry access.
...
```sh
node e2e/regressions/run.mjs --controls --output local/regressions/controls-01
```
...
```sh
node e2e/regressions/run.mjs --output local/regressions/candidate-01
git archive <baseline> skills | tar -x -C local/regressions/baseline-skills
node e2e/regressions/run.mjs --skills-root local/regressions/baseline-skills --output local/regressions/baseline-01
```
