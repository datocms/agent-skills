...
const { values } = parseArgs({ options: { 'skills-root': { type: 'string' }, fixtures: { type: 'string' }, dependencies: { type: 'string' }, output: { type: 'string' }, cases: { type: 'string', default: cases.map(c => c.id).join(',') }, recheck: { type: 'string' } } });
...
for (const c of selected) {
  ...
  const result = { case: c.id, model: 'gpt-5.6-luna', reasoningEffort: 'medium', passed: false, rubric: c.rubric };
  let session;
  try {
    if (!values.recheck) {
      ...
      session = await nativeSession({ repoRoot: resolve(values['skills-root']), workspace, output: directory, prompt: c.prompt, timeoutMs: 600000 });
