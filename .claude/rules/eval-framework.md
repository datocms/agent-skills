...
- Canonical results live at `evals/results/trigger/<skill>/<track>/<source>/results.json` and record the model that answered (Codex only when `--model` is pinned); cross-skill summary at `evals/results/trigger/_summary/<track>/<source>/summary.{json,md}`. Commit results only from a deliberate full run.
