...
DatoCMS skills repository — 10 public skills that provide focused guidance for content delivery, content management, content modeling, CLI workflows, frontend integrations, plugin development, plugin UI design, plugin scaffolding, project setup, and feedback drafting. Ships as static markdown; no build or bundle step.

## Repository Structure

- `.Codex-plugin/plugin.json` — Codex plugin manifest (points `skills` at `./skills/`)
- `.Codex-plugin/marketplace.json` — Codex marketplace registry (lists the `datocms` plugin for `/plugin` discovery)
